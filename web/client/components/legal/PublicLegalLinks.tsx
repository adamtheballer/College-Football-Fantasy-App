...
export function PublicLegalLinks({ className, includeDataSources = true }: PublicLegalLinksProps) {
  return (
    <nav aria-label="Legal information" className={cn("flex flex-wrap items-center justify-center gap-x-3 gap-y-2", className)}>
      ...
      {includeDataSources ? (
        <>
          <span aria-hidden="true">·</span>
          <Link to="/provider-disclosure" className="transition-colors hover:text-cfb-text-primary">
            Data Sources
          </Link>
        </>
      ) : null}
