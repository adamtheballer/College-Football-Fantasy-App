          ...
          {loading ? (
            ...
            <section className="rounded-3xl border border-white/10 bg-white/[0.045] p-5">
              ...
              : valueQuery.data?.current ? (() => {
                ...
                return <div className="mt-5 space-y-5"><div className="grid gap-5 md:grid-cols-[220px_1fr]"><div className="flex aspect-square w-full max-w-[220px] items-center justify-center rounded-full p-3" style={{ background: `conic-gradient(from -90deg, ${meterColor} 0deg ${meterDegrees}deg, rgba(148, 163, 184, 0.22) ${meterDegrees}deg 360deg)` }} aria-label={`Trade value ${meterValue.toFixed(0)} out of 99, ${value.tier.replace(/_/g, " ")}`}><div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-slate-950 text-center"><p className="text-5xl font-black tabular-nums text-white">{meterValue.toFixed(0)}</p><p className="mt-1 text-[10px] font-black uppercase tracking-[0.18em] text-white/55">out of 99</p><p className="mt-3 text-xs font-black uppercase tracking-[0.15em] text-cyan-200">{value.tier.replace(/_/g, " ")}</p></div></div><div className="grid content-center gap-3 sm:grid-cols-3"><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-white/45">Weekly change</p><p className="mt-2 text-xl font-black text-white">{change === null || change === undefined ? "NEW" : change > 0 ? `UP ${change}` : change < 0 ? `DOWN ${Math.abs(change)}` : "NO CHANGE"}</p></div><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-white/45">Value rank</p><p className="mt-2 text-xl font-black text-white">{position || "Player"} #{value.positional_value_rank ?? "—"}</p></div><div className="rounded-2xl border border-white/10 bg-black/20 p-3"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-white/45">Updated</p><p className="mt-2 text-sm font-black text-white">{new Date(value.calculated_at).toLocaleString()}</p></div></div></div><div className="rounded-2xl border border-cyan-200/15 bg-cyan-200/10 p-4"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100">Why value changed</p><div className="mt-3 space-y-2">{value.explanations.length ? value.explanations.map((item, index) => <p key={`${item.reason}-${index}`} className="text-sm font-bold text-white">{item.direction === "DOWN" ? "−" : "+"} {item.label}</p>) : <p className="text-sm font-bold text-white/65">Published preseason rating and current context remain stable.</p>}</div></div><div className="grid gap-3 md:grid-cols-2"><div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/55">Value breakdown</p><div className="mt-3 space-y-2">{Object.entries(value.factor_breakdown ?? {}).map(([label, amount]) => <div key={label} className="flex justify-between gap-3 text-sm"><span className="font-bold text-white/65">{label.replace(/([A-Z])/g, " $1")}</span><span className="font-black text-white">{Number(amount).toFixed(1)}</span></div>)}</div></div><div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-white/55">Value trend</p><div className="mt-4 flex items-end gap-2" aria-label="Weekly value trend">{trend.map((item) => <div key={item.week} className="flex flex-1 flex-col items-center gap-2"><div className="w-full rounded-t bg-cyan-300/80" style={{ height: `${Math.max(8, Math.min(99, item.value))}px` }} title={`Week ${item.week}: ${item.value}`} /><span className="text-[9px] font-black text-white/45">{item.week === 0 ? "PRE" : `W${item.week}`}</span></div>)}</div></div></div></div>;
