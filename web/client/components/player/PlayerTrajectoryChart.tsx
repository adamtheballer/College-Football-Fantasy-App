...
export function PlayerTrajectoryChart({
  ...
  const y = (value: number) =>
    PADDING.top + (1 - Math.min(finiteValue(value), yMax) / yMax) * plotHeight;
  ...
  const numericPoints = ordered.filter(
    (point): point is TrajectoryPoint & { value: number } =>
  ...
  const hasConnectedWeeks = numericPoints.some(
    (point, index) =>
      index > 0 && point.week === numericPoints[index - 1].week + 1,
  );
  const peak = numericPoints.reduce(
    (best, point) => (point.value > best.value ? point : best),
    numericPoints[0] ?? { week: 0, value: 0 },
  );
  const isPreseasonOnly = ordered.length === 1 && ordered[0]?.week === 0;
  const isCurrentProjectionOnly =
    isPreseasonOnly && ordered[0]?.source === "current";
  ...
  const pointLabel = (point: TrajectoryPoint) => {
    ...
    if (isProjection && point.source === "actual")
      return `Week ${point.week} actual fantasy points`;
    if (isProjection && point.source === "published")
      return `Week ${point.week} preweek baseline`;
    if (point.week === 0)
      return point.source === "current" ? "Current projection" : "Preseason";
  ...
  return (
    <section
      className="rounded-3xl border border-cyan-200/20 bg-[#091323] p-4 sm:p-5"
    ...
    >
      <div className="mb-4 flex flex-wrap items-end justify-between gap-2">
        ...
        {numericPoints.length ? (
          <p className="text-xs font-black text-white">
            Peak: {valueFormatter(peak.value)}{" "}
            <span className="text-white/45">
              ({peak.week === 0 ? "Preseason" : `W${peak.week}`})
            </span>
          </p>
        ) : null}
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[10px] font-bold text-white/50">
        {isProjection ? (
          ...
          <>
            <span>
              <span className="mr-1 inline-block h-2 w-2 rounded-full bg-cyan-300" />
              {points.some((point) => point.source === "current")
                ? "Current projection"
                : "Preseason baseline"}
            </span>
            <span>
        ...
        {points.some((point) => point.source === "bye") ? (
          <span>
