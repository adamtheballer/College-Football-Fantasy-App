...
type DraftSortMode = "draft_rank" | "projection" | "adp" | "name" | "position" | "school";
type DraftPositionFilter = "ALL" | "QB" | "RB" | "WR" | "TE" | "K";
type DraftActiveTab = "draft" | "queue" | "roster" | "history";
type DraftStatRow = { key: string; label: string; value: number };

const getStableRankNumber = (value: unknown) => {
...
export default function Draft() {
  ...
  const [activeTab, setActiveTab] = useState<DraftActiveTab>("draft");
  const [sortMode, setSortMode] = useState<DraftSortMode>("draft_rank");
  ...
  const { data: draftHistory } = useDraftHistory(parsedLeagueId, Boolean(draftRoom?.is_complete));
  ...
  const draftTimer = useDraftTimer({
    serverTime: draftRoom?.server_time,
    currentPickExpiresAt: draftRoom?.current_pick_expires_at,
    ...
    if (
      !isDraftAutoPickDue({
        draftRoom: {
          ...
          isComplete: draftRoom?.is_complete,
          currentTeamId: draftRoom?.current_team_id,
          phaseType: draftRoom?.phase_type,
        },
        isExpired: draftTimer.isExpired,
        autoPickPending: autoPickMutation.isPending,
      })
    ) {
      return;
    }
    void autoPickMutation.mutateAsync({ force: false }).catch(() => {
      void autoPickMutation.mutateAsync({ force: true }).catch(() => undefined);
    });
  }, [
    autoPickMutation,
    draftRoom?.current_pick,
    draftRoom?.current_team_id,
    draftRoom?.is_complete,
    draftRoom?.phase_type,
    draftRoom?.status,
    draftTimer.isExpired,
  ]);
  ...
  const boardPlayers = useMemo<DraftBoardPlayer[]>(() => {
    ...
    return [...offensePlayers]
      ...
      .map((player, index) => {
        const projectedStandardPoints = Number.isFinite(player.sheetProjectedSeasonPoints as number)
          ? Number(player.sheetProjectedSeasonPoints)
          : Number(player.projection?.fpts ?? 0);
        const adpFromSheet = Number.isFinite(player.sheetAdp as number) && Number(player.sheetAdp) > 0
          ? Number(player.sheetAdp)
          : 9999 + index;
        return {
          ...player,
          projectedStandardPoints,
          adpRank: adpFromSheet,
          draftRank: getStableRankNumber(player.sheetAdp) ?? getStableRankNumber(player.rank) ?? index + 1,
        };
      });
  }, [playersPayload?.data]);
  ...
  const effectiveSortMode: DraftSortMode = sortMode;

  const sortedBoardPlayers = useMemo(() => {
    const sorted = [...boardPlayers].sort((left, right) => {
      if (effectiveSortMode === "draft_rank" || effectiveSortMode === "adp") {
      ...
      if (effectiveSortMode === "projection" && right.projectedStandardPoints !== left.projectedStandardPoints) {
        return right.projectedStandardPoints - left.projectedStandardPoints;
      }
      if (effectiveSortMode === "name") return left.name.localeCompare(right.name);
      if (effectiveSortMode === "position") {
        const positionCompare = normalizePlayerPosition(left.pos).localeCompare(normalizePlayerPosition(right.pos));
        if (positionCompare !== 0) return positionCompare;
      }
      if (effectiveSortMode === "school") {
        const schoolCompare = String(left.school ?? "").localeCompare(String(right.school ?? ""));
    ...
    return sorted.map((player, index) => ({
      ...player,
      draftRank: getStableRankNumber(player.sheetAdp) ?? getStableRankNumber(player.rank) ?? getStableRankNumber(player.draftRank) ?? index + 1,
    }));
  ...
  const sendDraftHistoryEmail = async () => {
    setHistoryEmailError(null);
    try {
      await historyEmailMutation.mutateAsync({ send_to_account_email: true });
      setCompletionChoiceMade(true);
    } catch (error) {
      if (error instanceof ApiError && error.status === 503) {
        setHistoryEmailError("Email is not configured. You can copy the draft history and exit.");
        setCompletionChoiceMade(true);
        return;
      }
      setHistoryEmailError(formatApiError(error, "Unable to send draft history."));
    }
  };

  const skipDraftHistoryEmail = () => {
    setHistoryEmailError(null);
    setCompletionChoiceMade(true);
  };

  const copyDraftHistory = async () => {
    if (!draftHistory?.plain_text) return;
    await navigator.clipboard?.writeText(draftHistory.plain_text);
  };

  const exitCompletedDraft = () => {
    setCompletionModalDismissed(true);
    navigate(getCompletedDraftExitPath());
  };

  const showCompletionModal = shouldShowDraftCompletionModal({
    isComplete: draftRoom?.is_complete,
    dismissed: completionModalDismissed,
  });
  ...
  const liveTimerSeconds = Math.max(0, draftTimer.secondsRemaining);
  ...
  return (
    <div className="max-w-7xl mx-auto py-10 space-y-6">
      ...
      {showCompletionModal && (
        <div className="fixed inset-0 z-[220] flex items-center justify-center bg-slate-950/72 px-4 backdrop-blur-xl">
          <div className="w-full max-w-[560px] rounded-2xl border border-cyan-200/16 bg-[#08111f] p-6 text-center shadow-[0_24px_80px_rgba(0,0,0,0.48)]">
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-200/80">Draft Complete</p>
            <h2 className="mt-3 text-2xl font-black text-white">Want to send the draft history to your email?</h2>
            <p className="mt-3 text-sm font-semibold leading-6 text-slate-400">
              {draftHistory ? `${draftHistory.pick_count} picks are ready in the draft history.` : "Draft history is being prepared."}
            </p>
            {historyEmailError ? (
              <p className="mt-3 text-[10px] font-black uppercase tracking-[0.14em] text-amber-200">{historyEmailError}</p>
            ) : null}
            {!completionChoiceMade ? (
              <div className="mt-6 grid grid-cols-2 gap-3">
                <Button
                  type="button"
                  variant="outline"
                  className="h-11 rounded-xl text-[10px] font-black uppercase tracking-[0.16em]"
                  onClick={skipDraftHistoryEmail}
                >
                  No Thanks
                </Button>
                <Button
                  type="button"
                  className="h-11 rounded-xl bg-cyan-300 text-[10px] font-black uppercase tracking-[0.16em] text-slate-950 hover:bg-cyan-200"
                  disabled={historyEmailMutation.isPending}
                  onClick={() => void sendDraftHistoryEmail()}
                >
                  {historyEmailMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Send To My Email"}
                </Button>
              </div>
            ) : (
              <div className="mt-6 grid grid-cols-2 gap-3">
                <Button
                  ...
                  variant="outline"
                  className="h-11 rounded-xl text-[10px] font-black uppercase tracking-[0.16em]"
                  onClick={() => void copyDraftHistory()}
                  disabled={!draftHistory?.plain_text}
                >
                  Copy History
                </Button>
                <Button
        ...
        <CardContent className="p-5">
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as DraftActiveTab)}>
            <TabsContent value="draft" className="space-y-4">
              ...
              {(playersError || sheetSyncMutation.error) && (
                <div className="rounded-2xl border border-red-400/20 bg-red-500/5 p-4 text-[10px] font-black uppercase tracking-[0.18em] text-red-300">
                  {playersError
                    ? "Unable to load player pool."
                    : "Unable to sync Google Sheet player pool. Export the sheet as CSV and run the import script locally."}
                </div>
              )}
              ...
              <div className="overflow-hidden rounded-[2rem] border border-primary/20 bg-gradient-to-b from-white/[0.06] to-white/[0.03] shadow-[inset_0_0_28px_rgba(59,130,246,0.12)]">
                <div className="grid grid-cols-[68px_minmax(0,1fr)_90px_170px_110px_90px_170px_210px] gap-4 border-b border-white/10 px-5 py-3 text-[9px] font-black uppercase tracking-[0.28em] text-muted-foreground/70 sticky top-0 bg-[#071327]/95 backdrop-blur">
                  ...
                  <span>School</span>
                  <span>Class</span>
                  <span className={effectiveSortMode === "adp" || effectiveSortMode === "draft_rank" ? "text-primary" : ""}>ADP</span>
                  <span className={effectiveSortMode === "projection" ? "text-primary" : ""}>Proj Pts</span>
                ...
                <div ref={boardScrollRef} className="max-h-[75vh] overflow-y-auto">
                  {playersLoading ? (
                    ...
                    <div className="px-6 py-10 text-center text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">
                      {playersPayload?.total === 0 && !searchQuery.trim() && positionFilter === "ALL"
                        ? "No players have been imported yet. Run the Google Sheet import script to seed the draft player pool."
                        : searchQuery.trim() || positionFilter !== "ALL"
                          ...
                          <div className="flex flex-col items-end gap-1">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <Button
                                type="button"
                                className="h-9 rounded-xl text-[9px] font-black uppercase tracking-[0.18em]"
                                disabled={!draftRoom.can_make_pick || pickMutation.isPending || autoPickMutation.isPending || draftedIds.has(player.id) || !playerCanFitRoster}
                              ...
                              >
                                {pickMutation.isPending || autoPickMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : draftedIds.has(player.id) ? "Drafted" : "Draft"}
                              </Button>
                            </div>
                            {rosterLockReason && (
              ...
              <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={!draftRoom.can_make_pick || draftableQueuedPlayers.length === 0 || pickMutation.isPending || autoPickMutation.isPending}
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 border-t border-white/10 bg-[#0B1A35] px-6 py-4">
              ...
              <Button
                ...
                disabled={!draftRoom.can_make_pick || pickMutation.isPending || autoPickMutation.isPending || selectedPlayerDrafted || !selectedPlayerCanFitRoster}
