...
export default function ResetPassword() {
  ...
  const field = (id: string, label: string, value: string, setValue: (value: string) => void, shown: boolean, setShown: (value: boolean) => void) => <div className="block text-[10px] font-black uppercase tracking-widest text-cfb-text-muted"><label htmlFor={id}>{label}</label><span className="relative mt-2 block"><Input id={id} type={shown ? "text" : "password"} autoComplete="new-password" value={value} onChange={(event) => setValue(event.target.value)} className="h-14 rounded-2xl pr-12 text-base" required /><button type="button" aria-label={shown ? `Hide ${label.toLowerCase()}` : `Show ${label.toLowerCase()}`} onClick={() => setShown(!shown)} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-cfb-text-muted">{shown ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></span></div>;
