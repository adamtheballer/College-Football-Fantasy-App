...
export const buildScorePreviewRows = (
  preview: CorrectionPreview | null,
): ScorePreviewRow[] => {
  if (!preview) return [];
...
export const buildStatPreviewRows = (
  preview: CorrectionPreview | null,
): StatPreviewRow[] => {
  if (!preview) return [];
...
export default function AdminScoring() {
  ...
  const [statsJson, setStatsJson] = useState(
    '{"PassingYards":300,"PassingTouchdowns":3}',
  );
  ...
  const healthQuery = useQuery({
    ...
    queryFn: () =>
      apiGet<ProviderHealthResponse>("/admin/scoring/provider-health"),
  });
  const runsQuery = useQuery({
    queryKey: ["admin", "scoring", "runs", "failed"],
    enabled: !!user?.isAdmin,
    queryFn: () =>
      apiGet<RunsResponse>("/admin/scoring/runs", {
        status: "failed",
        limit: 25,
      }),
  });
  const historyQuery = useQuery({
    ...
    enabled: !!user?.isAdmin,
    queryFn: () =>
      apiGet<AuditRow[]>("/admin/scoring/corrections", { limit: 25 }),
  });
  ...
  const mutation = useMutation({
    mutationFn: async ({ path, body }: { path: string; body: unknown }) =>
      apiPost<AdminActionResponse>(path, body),
    onSuccess: (payload) => {
  ...
  const scorePreviewRows = useMemo(
    () => buildScorePreviewRows(preview),
    [preview],
  );
  const statPreviewRows = useMemo(
    () => buildStatPreviewRows(preview),
    [preview],
  );
  ...
  const baseInputsAreValid =
    Number.isFinite(numericSeason) &&
    Number.isFinite(numericWeek) &&
    numericWeek > 0 &&
    reasonIsValid;
  const correctionInputsAreValid =
    baseInputsAreValid &&
  ...
  const handlePreview = async () => {
    try {
      ...
      const payload = await apiPost<CorrectionPreview>(
        "/admin/scoring/corrections/preview",
        body,
      );
      setPreview(payload);
      toast({
        title: "Correction preview ready",
        description: `${payload.affected_league_ids.length} affected league(s).`,
      });
    } catch (error) {
      toast({
  ...
  const handleApply = () => {
    queueAction({
      ...
      description:
        "This will update the provider stat row, recalculate every affected league, and write an audit event.",
      execute: () => {
        mutation.mutate({
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1320px] flex-col gap-6 px-6 py-8">
      <div>
        ...
        <h1 className="mt-2 text-4xl font-black italic text-slate-50">
          Scoring Repair Tools
        </h1>
        <p className="mt-2 max-w-3xl text-sm font-semibold text-slate-400">
          Controlled scoring reruns, stat corrections, reconciliation, week
          finalization, and provider-health inspection.
        </p>
      </div>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <div className="rounded-[1.5rem] border border-sky-300/20 bg-sky-400/10 p-5">
          ...
          <p className="mt-1 text-3xl font-black text-slate-50">
            {healthQuery.data?.failed_scoring_runs ?? "—"}
          </p>
        </div>
        <div className="rounded-[1.5rem] border border-amber-300/20 bg-amber-400/10 p-5">
          ...
          <p className="mt-1 text-3xl font-black text-slate-50">
            {healthQuery.data?.open_unmatched_rows ?? "—"}
          </p>
        </div>
        <div className="rounded-[1.5rem] border border-emerald-300/20 bg-emerald-400/10 p-5">
          ...
          <p className="mt-1 text-3xl font-black text-slate-50">
            {healthQuery.data?.sync_states.length ?? "—"}
          </p>
        </div>
      </section>
        ...
        <div className="rounded-[2rem] border border-sky-300/20 bg-slate-950/55 p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Button
              ...
              disabled={!correctionInputsAreValid}
            >
              Preview Correction
            </Button>
            <Button
              ...
              onClick={handleApply}
              disabled={
                mutation.isPending || !correctionInputsAreValid || !preview
              }
            >
              Apply Correction
            </Button>
            <Button
              ...
              onClick={() => {
                queueAction({
                  label: "Rerun scoring",
                  description:
                    "This recalculates scoring for the selected league/week and records an audit event.",
                  execute: () =>
                    mutation.mutate({
                      path: "/admin/scoring/rerun",
                      body: { ...basePayload, provider: "admin" },
                    }),
                });
              }}
            >
            ...
            <Button
              ...
              disabled={
                mutation.isPending ||
                !baseInputsAreValid ||
                !basePayload.league_id
              }
              onClick={() => {
                queueAction({
                  label: "Reconcile league/week",
                  description:
                    "This recomputes league totals for the selected week and records an audit event.",
                  execute: () =>
                    mutation.mutate({
                      path: "/admin/scoring/reconcile/league-week",
                      body: basePayload,
                    }),
                });
              }}
            ...
            <Button
              ...
              onClick={() => {
                queueAction({
                  ...
                  description:
                    "This recomputes every affected league for this player/week and records an audit event.",
                  execute: () =>
                    mutation.mutate({
            ...
            <Button
              ...
              onClick={() => {
                queueAction({
                  label: "Finalize week",
                  description:
                    "This marks all matchups for the selected league/week as final.",
                  execute: () =>
                    mutation.mutate({
                      path: "/admin/scoring/weeks/finalize",
                      body: basePayload,
                    }),
                });
              }}
            ...
            <Button
              ...
              onClick={() => {
                queueAction({
                  label: "Reopen week",
                  description:
                    "This reopens all matchups for the selected league/week for controlled correction.",
                  execute: () =>
                    mutation.mutate({
                      path: "/admin/scoring/weeks/reopen",
                      body: basePayload,
                    }),
                });
              }}
          ...
          {pendingAction ? (
            ...
            >
              <h3
                id="admin-action-confirm-title"
                className="text-sm font-black uppercase tracking-[0.18em] text-amber-100"
              >
              ...
              <p className="mt-3 text-xs font-bold text-amber-50/70">
                Reason: {reason.trim()}
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                <Button
        ...
        <div className="rounded-[2rem] border border-sky-300/20 bg-slate-950/55 p-6">
          ...
          {preview ? (
            <div className="mt-4 space-y-5">
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-sky-300/10 bg-sky-400/10 p-4">
                  ...
                  <p className="mt-1 text-xl font-black text-slate-50">
                    {preview.season} · Week {preview.week}
                  </p>
                </div>
                <div className="rounded-2xl border border-sky-300/10 bg-sky-400/10 p-4">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">
                  ...
                  <p className="mt-1 text-xl font-black text-slate-50">
                    {preview.affected_league_ids.length}
                  </p>
                </div>
              </div>
              ...
              <div className="overflow-hidden rounded-2xl border border-sky-300/10">
                ...
                {scorePreviewRows.map((row) => (
                  <div
                    key={row.leagueId}
                    className="grid grid-cols-4 border-t border-sky-300/10 px-4 py-3 text-sm font-bold text-slate-200"
                  >
              ...
              <div className="overflow-hidden rounded-2xl border border-sky-300/10">
                ...
                {statPreviewRows.map((row) => (
                  <div
                    key={row.key}
                    className="grid grid-cols-[1fr_1fr_1fr] border-t border-sky-300/10 px-4 py-3 text-sm font-bold text-slate-200"
                  >
                    <span>{row.key}</span>
            ...
            <p className="mt-4 text-sm font-semibold text-slate-500">
              Run a preview before applying any correction.
            </p>
          )}
        </div>
      </section>
        ...
        <div className="rounded-[2rem] border border-red-300/20 bg-red-500/5 p-6">
          ...
          <div className="mt-4 space-y-3">
            {(runsQuery.data?.data ?? []).map((run) => (
              <div
                key={run.id}
                className="rounded-2xl border border-red-300/10 bg-slate-950/50 p-4 text-sm text-slate-300"
              >
                <p className="font-black text-red-100">
                  Run #{run.id} · League {run.league_id ?? "Global"} · Week{" "}
                  {run.week}
                </p>
                ...
                <p className="mt-2 text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">
                  Started {formatDateTime(run.started_at)} · Provider{" "}
                  {run.provider}
                </p>
              </div>
            ))}
        ...
        <div className="rounded-[2rem] border border-sky-300/20 bg-slate-950/55 p-6">
          ...
          <div className="mt-4 space-y-3">
            {(historyQuery.data ?? []).map((row) => (
              <div
                key={row.id}
                className="rounded-2xl border border-sky-300/10 bg-slate-950/50 p-4 text-sm text-slate-300"
              >
                <p className="font-black text-sky-100">
                  Correction #{row.id} · Player {row.player_id} · Week{" "}
                  {row.week}
                </p>
