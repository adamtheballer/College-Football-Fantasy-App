...
import {
  useFinalizeLeagueWeek,
  useGenerateSchedule,
  useScoreLeagueWeek,
} from "@/hooks/use-game-loop";
...
export default function LeagueDetail() {
  ...
  const [activeHubTab, setActiveHubTab] = useState<
    "my-team" | "lineup" | "matchup" | "scoreboard" | "standings" | "league-news" | "commissioner"
  >("my-team");
  ...
  const controlWeek = selectedWeek ?? workspace?.matchup_summary?.week ?? 1;
  const generateSchedule = useGenerateSchedule(parsedLeagueId ?? 0);
  const scoreWeek = useScoreLeagueWeek(parsedLeagueId ?? 0, controlWeek);
  const finalizeWeek = useFinalizeLeagueWeek(parsedLeagueId ?? 0, controlWeek);
  ...
  const hubTabs = useMemo(
    () => [
      ...
      { id: "commissioner", label: "Commissioner", icon: ShieldCheck },
    ] as const,
    []
  );
    ...
    <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-700 pb-16">
      ...
      <Card className="bg-card/40 border border-white/10 rounded-[2.5rem] overflow-hidden">
        ...
        <CardContent className="p-6 space-y-6">
          ...
          {activeHubTab === "lineup" && (
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5 space-y-4">
              ...
              <p className="text-sm text-muted-foreground/80">
                Save your weekly snapshot before the lineup locks. This snapshot is what scoring uses, not your future roster state.
              </p>
              <Button asChild className="rounded-2xl text-[10px] font-black uppercase tracking-[0.18em]">
                <Link to={`/league/${league.id}/lineup`}>Open Lineup</Link>
              </Button>
            </div>
          )}
              ...
              {scoreboardQuery.isLoading ? (
                ...
                <div className="space-y-2">
                  {scoreboardRows.map((row) => (
                    <Link
                      key={row.matchup_id}
                      to={`/league/${league.id}/matchup/${row.matchup_id}`}
          ...
          {activeHubTab === "standings" && (
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5 space-y-4">
              <p className="text-[10px] font-black uppercase tracking-[0.22em] text-primary">
                Standings
              </p>
              {workspace?.standings_summary.length ? (
                <div className="space-y-2">
                  {workspace.standings_summary.map((row) => (
                    <div key={row.team_id} className="grid grid-cols-[60px_1fr_120px_120px] gap-3 rounded-xl border border-white/10 bg-black/10 px-4 py-3 text-sm">
                      <span className="font-black text-primary">#{row.rank ?? "-"}</span>
                      <span className="font-black text-foreground">{row.team_name}</span>
                      <span>{row.wins ?? 0}-{row.losses ?? 0}-{row.ties ?? 0}</span>
                      <span>PF {(row.points_for ?? 0).toFixed(1)}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-[11px] text-muted-foreground/70">Standings appear after a week is finalized.</p>
              )}
            </div>
          )}
          ...
          {activeHubTab === "commissioner" && (
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5 space-y-4">
              <p className="text-[10px] font-black uppercase tracking-[0.22em] text-primary">
                Commissioner Controls
              </p>
              {!isCommissioner ? (
                <p className="text-[11px] text-muted-foreground/70">Only the commissioner can generate schedules, score weeks, or finalize weeks.</p>
              ) : (
                <>
                  <div className="flex flex-wrap gap-3">
                    <Button onClick={() => generateSchedule.mutate()} disabled={generateSchedule.isPending} className="rounded-2xl text-[10px] font-black uppercase tracking-[0.18em]">
                      Generate Schedule
                    </Button>
                    <Button onClick={() => scoreWeek.mutate()} disabled={scoreWeek.isPending} variant="outline" className="rounded-2xl text-[10px] font-black uppercase tracking-[0.18em]">
                      Recalculate Week {controlWeek}
                    </Button>
                    <Button onClick={() => finalizeWeek.mutate()} disabled={finalizeWeek.isPending} variant="outline" className="rounded-2xl text-[10px] font-black uppercase tracking-[0.18em]">
                      Finalize Week {controlWeek}
                    </Button>
                  </div>
                  {(generateSchedule.error || scoreWeek.error || finalizeWeek.error) && (
                    <p className="text-xs text-red-300">
                      {(generateSchedule.error || scoreWeek.error || finalizeWeek.error)?.message}
                    </p>
                  )}
                  {(generateSchedule.isSuccess || scoreWeek.isSuccess || finalizeWeek.isSuccess) && (
                    <p className="text-xs text-emerald-300">Commissioner action completed.</p>
                  )}
                </>
              )}
            </div>
