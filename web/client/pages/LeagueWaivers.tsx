...
type WaiverSortMode = "projection" | "positional-rank";
...
export const waiverPositionRankLabel = (player: Pick<AvailablePlayerRow, "position" | "season_positional_rank">) => (
  typeof player.season_positional_rank === "number" && player.season_positional_rank > 0
    ? `${player.position?.toUpperCase() ?? "POS"} ${player.season_positional_rank}`
    : "—"
);
...
export const sortWaiverPlayersByPositionRank = <T extends Pick<AvailablePlayerRow, "name" | "id" | "season_positional_rank" | "rank">>(
  ...
  const leftRank = left.season_positional_rank ?? Number.POSITIVE_INFINITY;
  const rightRank = right.season_positional_rank ?? Number.POSITIVE_INFINITY;
...
export default function LeagueWaivers() {
  ...
  const openEditClaimDialog = (claim: NonNullable<typeof waiverData>["claims"][number]) => {
    setClaimPlayer({
      ...
      injury_status: null,
      season_positional_rank: null,
      season_rank_through_week: null,
      ...
      <div className="space-y-3 sm:space-y-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <p className="text-xs font-semibold text-cfb-text-secondary sm:hidden">
            {players.length} {playerBoardScope === "all" ? "players" : playerBoardScope === "hot" ? "hot" : "available"} · Top proj {topProjection?.toFixed(1) ?? "—"} · {waiverData?.claims.length ?? 0} claims
          </p>
          <div className="hidden overflow-hidden rounded-lg border border-cfb-border-subtle bg-cfb-surface-raised sm:grid sm:min-w-[390px] sm:grid-cols-3">
            <div className="px-4 py-3">
              <p className="text-[9px] font-black uppercase tracking-[0.14em] text-cfb-text-muted">{playerBoardScope === "all" ? "Players" : playerBoardScope === "hot" ? "Hot" : "Available"}</p>
              ...
              <div className="grid w-full shrink-0 grid-cols-3 rounded-md border border-cfb-border-subtle bg-cfb-canvas p-1 sm:w-[360px]" role="tablist" aria-label="Player board">
                {([
                  ...
                  <button
                    ...
                    onClick={() => {
                      setPlayerBoardScope(scope);
                      if (scope === "hot") setSortMode("projection");
                    }}
                    className={[
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    {player.rostered_by_team_name ? (
                      <p className="mt-0.5 truncate text-[10px] font-bold uppercase tracking-[0.1em] text-cfb-text-muted">
                      ...
                      <td className="px-4 py-3 align-middle">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.12em] text-cfb-text-muted">
                            {player.rostered_by_team_name
                              ? `Rostered by ${player.rostered_by_team_name}`
                              : player.hot_pickup_count
                                ? `${player.hot_pickup_count} league pickup${player.hot_pickup_count === 1 ? "" : "s"}`
                                : "Available player"}
                          </p>
                          <PlayerPopularityMetrics popularity={player.popularity} />
                        </div>
