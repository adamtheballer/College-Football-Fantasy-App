...
const roadmap = [
  ...
  },
  {
    ...
    description: "A deeper player view that tracks your history drafting, starting, adding, and trading each player.",
    icon: Sparkles,
  },
  {
    title: "Sponsored Deals",
    description: "In-app partner offers and discount-code rewards, including Saturday Pick 6 promotions on the Home dashboard.",
    icon: Handshake,
  },
];
      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-cfb-brand/35 bg-cfb-surface-raised p-7 shadow-[0_0_52px_rgba(37,99,235,0.12)] sm:p-10">
        ...
        <div className="relative max-w-2xl">
          ...
          <p className="mt-4 text-base font-semibold leading-7 text-cfb-text-secondary sm:text-lg">
            A look at what we are building next. Saturday Pick 6 is available from the Home dashboard.
          </p>
        </div>
      </section>
      ...
      <section className="grid gap-5 md:grid-cols-3">
        {roadmap.map(({ title, description, icon: Icon }) => (
          <SurfaceCard key={title} variant="default" padding="default" className="min-h-56">
            ...
            <h2 className="mt-6 text-xl font-black text-cfb-text-primary">{title}</h2>
            <p className="mt-3 font-medium leading-6 text-cfb-text-secondary">{description}</p>
          </SurfaceCard>
        ))}
      </section>
    </div>
