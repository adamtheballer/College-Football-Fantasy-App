      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-cyan-300/30 bg-[radial-gradient(circle_at_20%_20%,rgba(14,165,233,0.22),transparent_32%),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(30,41,59,0.96),rgba(8,47,73,0.88))] p-6 shadow-[0_0_60px_rgba(14,165,233,0.14)] sm:p-9">
        ...
        <div className="relative flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-5">
            ...
            <div className="flex flex-wrap items-center gap-3"><div className="rounded-2xl border border-cfb-border-strong bg-slate-950/55 px-5 py-3 text-right"><p className="cfb-micro-label text-cfb-text-muted">{isOpen ? "Locks in" : "Contest status"}</p><p className="mt-1 font-display text-2xl font-black tabular-nums text-cyan-100">{isOpen ? countdown.value : statusLabel(contest.status)}</p></div>{embedded && isOpen ? <Button type="button" onClick={() => setPickDialogOpen(true)}>{contest.entry ? "Update Your Pick" : "Make Your Pick"}</Button> : null}</div>
          </div>
        </div>
      </section>

      {embedded ? <Dialog open={pickDialogOpen} onOpenChange={setPickDialogOpen}><DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto border-cyan-300/35 bg-[#081321] text-cfb-text-primary"><DialogHeader><DialogTitle className="text-3xl font-black uppercase italic">{contest.entry ? "Update Your Pick" : "Make Your Pick"}</DialogTitle><DialogDescription className="text-base font-semibold leading-6 text-cfb-text-secondary">Choose the featured {positionLabel(contest.contest_position)} you think will score the most fantasy points. You can change your selection until kickoff.</DialogDescription></DialogHeader><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{players.map((player) => { const selected = selectedPickId === player.id; return <button key={player.id} type="button" onClick={() => setPendingPickId(player.id)} className={`rounded-2xl border p-4 text-left transition ${selected ? "border-cyan-200 bg-cyan-300/[0.14] shadow-[0_0_24px_rgba(34,211,238,0.16)]" : "border-white/10 bg-black/20 hover:border-cyan-200/45 hover:bg-cyan-300/[0.06]"}`}><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-lg font-black text-white">{player.player_name}</p><p className="mt-1 text-[10px] font-black uppercase tracking-[0.14em] text-white/55">{player.school} · {player.canonical_position}</p></div>{selected ? <Check className="h-5 w-5 shrink-0 text-cyan-100" /> : null}</div><div className="mt-4 flex items-end justify-between gap-3"><div><p className="cfb-micro-label">Opponent</p><p className="mt-1 font-bold text-white/80">vs. {player.opponent}</p></div><div className="text-right"><p className="cfb-micro-label">Projected</p><p className="mt-1 text-xl font-black tabular-nums text-cyan-100">{formatPoints(player.projected_points)}</p></div></div><p className="mt-4 text-xs font-bold text-white/55">{formatKickoff(player.game_time)}</p></button>; })}</div>{submitError ? <p role="alert" className="text-sm font-bold text-red-300">{submitError}</p> : null}<DialogFooter><Button type="button" variant="outline" onClick={() => setPickDialogOpen(false)}>Cancel</Button><Button type="button" disabled={!selectedPickId || savePick.isPending} onClick={submit}>{savePick.isPending ? "Saving…" : contest.entry ? "Update Pick" : "Lock In Pick"}</Button></DialogFooter></DialogContent></Dialog> : <>
