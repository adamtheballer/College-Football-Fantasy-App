...
vi.mock("@/hooks/use-leagues", () => ({
  ...
  useUpdateDraftOrder: () => ({
    mutateAsync: state.orderMutateAsync,
    isPending: false,
  }),
...
describe("DraftLobby rescheduling", () => {
  it("keeps countdown values and unit labels inside four compact mobile timer cards", () => {
    ...
    expect(screen.getByLabelText("Days").className).toContain(
      "whitespace-nowrap",
    );
    expect(screen.getByLabelText("Minutes").className).toContain(
      "whitespace-nowrap",
    );
  });
  ...
  it("shows the commissioner a visible league-timezone modal with the current schedule prefilled", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Update Draft Time" }),
    ).toBeTruthy();
    expect((screen.getByLabelText("New date") as HTMLInputElement).value).toBe(
      "2026-08-20",
    );
    expect((screen.getByLabelText("New time") as HTMLInputElement).value).toBe(
      "19:30",
    );
    expect(screen.getAllByText(/America\/New_York/).length).toBeGreaterThan(0);
    expect(
      screen.getByRole("button", { name: "Save New Draft Time" }),
    ).toBeTruthy();
  ...
  it("keeps the modal open and shows a controlled API error", async () => {
    state.mutateAsync.mockRejectedValueOnce(
      new Error("Draft has already started."),
    );
    renderLobby();
    fireEvent.click(screen.getByRole("button", { name: "Reschedule Draft" }));
    fireEvent.click(
  ...
  it("keeps editing controls away from ordinary members and started drafts", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Reschedule Draft" }),
    ).toBeNull();

    state.userId = 1;
    state.league = makeLeague({
      draft: { ...makeLeague().draft, status: "pre_draft" },
    });
    rerender(
      <MemoryRouter>
        <DraftLobby />
