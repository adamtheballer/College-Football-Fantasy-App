...
const formatRecord = (
  summary: LeagueListCurrentUserSummary | null | undefined,
) => {
              ...
              <div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-lg border border-primary/30 bg-primary/10 text-primary">
                {leagueImageUrl && !iconFailed ? (
                  ...
                  <Trophy
                    ...
                    aria-label="Default league trophy"
                  />
                )}
              </div>
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={(event) =>
                        copyInviteValue(event, "code", inviteCode)
                      }
                    ...
                    <button
                      type="button"
                      onClick={(event) =>
                        copyInviteValue(event, "link", inviteLink)
                      }
          ...
          <div className="space-y-3">
            ...
            <div>
              <div className="grid gap-2 sm:grid-cols-3">
                ...
                <div className="rounded-lg border border-white/10 bg-black/15 p-3">
                  <p className="text-[10px] font-semibold tracking-[0.1em] uppercase text-muted-foreground">
                    {currentUserSummary?.matchup_week
                      ? `Week ${currentUserSummary.matchup_week}`
                      : "Matchup"}
                  </p>
                  <p className="mt-1.5 truncate text-sm font-semibold text-foreground">
                    {currentUserSummary?.opponent_team_name ||
                      "Schedule pending"}
                  </p>
                  <p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-sky-200">
                    Win chance{" "}
                    {formatWinProbability(
                      currentUserSummary?.win_probability_for,
                    )}
                  </p>
                </div>
              </div>
          ...
          {shouldShowDraftAction && (
            ...
            >
              ...
              {draftUnlocked ? (
                ...
                <LockKeyhole className="w-3 h-3 ml-2" />
              )}
            </Button>
          )}
          ...
          {isError && (
            <Card className="rounded-xl border-border/60 bg-[#15181c] p-6 text-center">
              <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-red-300">
                Unable to load leagues. Confirm the backend is running and your
                session is valid.
              </p>
              <Button
                variant="outline"
                onClick={() => void refetch()}
                className="mt-5 rounded-lg border-sky-300/25 text-sky-100"
              >
                Try Again
              </Button>
            </Card>
            ...
            <Card className="space-y-5 rounded-xl border-border/60 bg-[#15181c] p-6">
              <div className="space-y-2 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Create a league or join one with an invite code to get
                  started.
                </p>
              </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Card className="rounded-xl border-border/60 bg-[#15181c] p-6 text-center">
            <div className="space-y-4">
              ...
              <Button
                asChild
                className="h-11 w-full rounded-lg text-[10px] font-bold uppercase tracking-[0.1em]"
              >
          ...
          <Card className="rounded-xl border-border/60 bg-[#15181c] p-6 text-center">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="mx-auto max-w-[240px] text-sm text-muted-foreground">
                  Join an existing league with an invite code and start
                  scouting.
                </p>
              </div>
              <Button
                asChild
                className="h-11 w-full rounded-lg bg-emerald-500 text-[10px] font-bold uppercase tracking-[0.1em] text-white hover:bg-emerald-400"
              >
                <Link to="/login" className="block">
                  Sign In to Join
                </Link>
