...
export default function BetaAccess() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      navigate(reservation.existingAccount ? "/login" : "/signup", {
        replace: true,
        state: reservation.existingAccount
          ? { betaAccessPending: true }
          : undefined,
      });
    } catch (caught) {
  ...
  return (
    <main className="relative mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-3xl items-center justify-center overflow-hidden px-4 py-10">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -left-28 top-12 h-72 w-72 rounded-full bg-cyan-300/15 blur-3xl"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -right-24 bottom-6 h-80 w-80 rounded-full bg-violet-500/15 blur-3xl"
      />
        ...
        <div className="space-y-7 p-7 sm:p-10">
          <div className="space-y-4 text-center">
            ...
            <div>
              ...
              <h1 className="mt-3 text-4xl font-black uppercase italic tracking-tight text-white sm:text-5xl">
                Early Access
              </h1>
              <p className="mx-auto mt-3 max-w-lg text-sm font-medium leading-6 text-slate-300">
                Enter the email address and early-access code you received to
          ...
          <form className="space-y-4" onSubmit={submit}>
            <label className="block space-y-2">
              ...
              <span className="relative block">
                <Mail
                  className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-cyan-200/70"
                  aria-hidden="true"
                />
                <Input
                  type="email"
            ...
            {error ? (
              <p
                role="alert"
                className="rounded-2xl border border-rose-300/30 bg-rose-500/15 px-4 py-3 text-sm font-bold text-rose-100"
              >
                {error}
              </p>
            ) : null}
            <Button
              className="h-14 w-full rounded-2xl bg-gradient-to-r from-cyan-300 via-sky-400 to-blue-500 text-[11px] font-black uppercase tracking-[0.22em] text-slate-950"
          ...
          <div className="space-y-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-center">
            <p className="flex items-center justify-center gap-2 text-xs font-semibold text-slate-300">
              <LockKeyhole className="h-4 w-4 text-cyan-200" /> Your code is
              validated securely and never displayed again.
            </p>
            <p className="text-xs text-slate-400">
              Use the same email address where you received your code.
            </p>
            <p className="text-xs text-slate-400">
              Sign in if you already created your account and entered your code.
            </p>
            <Link
              to="/login"
              className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-cyan-200 hover:text-cyan-100"
            >
