        ...
        <section className="hidden space-y-7 pl-2 lg:block xl:pl-8">
          ...
          <div className="grid max-w-2xl grid-cols-3 gap-3">
            {[
              ...
              <div key={item.label} className={`rounded-2xl border border-white/12 bg-gradient-to-br ${item.tone} p-4 shadow-[0_18px_40px_rgba(0,0,0,0.22)]`}>
          ...
          <div className="flex max-w-xl flex-wrap gap-3">
            {["CFB rankings", "Rivalry week", "Waiver wire", "Draft room"].map((label) => (
              <span key={label} className="rounded-full border border-white/12 bg-white/[0.08] px-4 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white/75">
                {label}
              </span>
            ))}
          </div>
