...
export const initialLoginMode = (
  flow: string | null,
  earlyAccessProEnabled: boolean,
): LoginMode => {
  ...
  return (flow === "pro" || flow === "beta") && earlyAccessProEnabled
    ? "access"
    : "signin";
};
...
export default function Login() {
  ...
  const signupHasVerifiedProCode =
    activeReservation?.email === email.trim().toLowerCase()
      ? activeReservation
      : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const matchingReservation =
        reservation && reservation.email === email.trim().toLowerCase()
          ? reservation.token
          : undefined;
  ...
  const handleAccessSubmit = async (event: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const reservation = await validateBetaAccess(
        email,
        betaAccessCodeFromSuffix(accessCode),
      );
      ...
      if (reservation.existingAccount) {
        selectMode("signin");
        setError(
          "Your Early Access Pro code was verified. Sign in with that same email to record your free year for alpha.",
        );
      } else {
        setMode("signup");
      }
      ...
      if (caught instanceof ApiError && caught.status === 0) {
        ...
        setError(
          "Unable to verify your Early Access Pro code. Please try again.",
        );
      }
    } finally {
      setIsLoading(false);
  ...
  if (
    shouldHoldAuthEntry(isBootstrapping, isLoggedIn, holdPostSignupRedirect)
  ) {
    return (
  ...
  return (
    <main className="relative mx-auto grid min-h-[calc(100vh-8rem)] w-full max-w-7xl items-center gap-6 overflow-hidden px-4 py-8 sm:px-6 lg:grid-cols-[1.02fr_0.98fr] lg:px-8">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -left-20 top-10 h-32 w-96 rotate-[-18deg] rounded-full bg-gradient-to-r from-cfb-pink/35 via-cfb-brand/30 to-transparent blur-2xl"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -right-24 top-24 h-32 w-[30rem] rotate-[-16deg] rounded-full bg-gradient-to-r from-transparent via-cfb-cyan/30 to-cfb-gold/24 blur-2xl"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute bottom-8 left-20 h-24 w-[26rem] rotate-[-10deg] rounded-full bg-gradient-to-r from-cfb-gold/24 via-cfb-brand/18 to-transparent blur-2xl"
      />
      ...
      <section className="relative hidden lg:block">
        <SurfaceCard
          variant="scoreboard"
          padding="spacious"
          className="cfb-playbook-pattern min-h-[560px]"
        >
            ...
            <div className="space-y-7">
              ...
              <div className="max-w-2xl space-y-4">
                ...
                <p className="max-w-xl text-base font-semibold leading-7 text-cfb-text-secondary">
                  Sign in to manage your leagues, draft rooms, rosters, alerts,
                  and matchup decisions from one college football command
                  center.
                </p>
              </div>
            </div>
      ...
      <section className="relative mx-auto w-full max-w-[520px]">
        ...
        >
          ...
          <div className="relative space-y-8 p-6 sm:p-8">
            ...
            {mode === "signin" ? (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-2">
                  <label
                    htmlFor="login-email"
                    className="ml-3 block text-[10px] font-black uppercase tracking-widest text-cfb-text-muted"
                  >
                    Email address
                  </label>
                  <span className="group relative block">
                    ...
                    <Input
                      ...
                      className="h-14 rounded-2xl border-cfb-border-subtle bg-cfb-surface/80 pl-12 text-base font-bold text-cfb-text-primary placeholder:text-cfb-text-muted transition focus:border-cfb-brand/60 focus:ring-cfb-brand/25 md:text-sm"
                      value={email}
                ...
                <div className="space-y-2">
                  <span className="flex items-center justify-between px-3">
                    <label
                      ...
                      className="text-[10px] font-black uppercase tracking-widest text-cfb-text-muted"
                    >
                      Password
                    </label>
                    {emailEnabled ? (
                      <Link
                        to="/reset-password"
                        className="text-[9px] font-black uppercase tracking-widest text-cfb-gold transition hover:text-yellow-100"
                      >
                        Reset password
                      </Link>
                    ) : null}
                  ...
                  <span className="group relative block">
                    ...
                    <Input
                      ...
                      placeholder="••••••••"
                      className="h-14 rounded-2xl border-cfb-border-subtle bg-cfb-surface/80 pl-12 pr-12 text-base font-bold text-cfb-text-primary placeholder:text-cfb-text-muted transition focus:border-cfb-brand/60 focus:ring-cfb-brand/25 md:text-sm"
                      value={password}
                ...
                {betaAccessPending ? (
                  <div className="rounded-2xl border border-cfb-cyan/35 bg-cfb-cyan/[0.10] px-4 py-3 text-xs font-bold text-cyan-50">
                    Your Early Access Pro code was verified. Sign in with that
                    same email to record your free year for alpha.
                  </div>
                ) : null}
                {error ? (
                  <div
                    role="alert"
                    className="rounded-2xl border border-cfb-danger/35 bg-cfb-danger/[0.14] px-4 py-3 text-xs font-bold text-red-100"
                  >
                    {error}
                  </div>
                ) : null}
              ...
              <form onSubmit={handleAccessSubmit} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <span className="group relative block">
                    <Mail className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-cfb-text-muted" />
                    <Input
                      id="beta-email"
                      type="email"
                      autoComplete="email"
                      placeholder="coach@saturday.com"
                      className="h-14 rounded-2xl border-cfb-border-subtle bg-cfb-surface/80 pl-12 text-base font-bold text-cfb-text-primary md:text-sm"
                      value={email}
                ...
                <div className="space-y-2">
                  ...
                  <span className="group flex h-14 items-center rounded-2xl border border-cfb-border-subtle bg-cfb-surface/80 transition focus-within:border-cfb-brand/60 focus-within:ring-2 focus-within:ring-cfb-brand/25">
                    <span
                      aria-hidden="true"
                      className="ml-2 inline-flex h-10 shrink-0 items-center gap-2 rounded-xl bg-cfb-brand/[0.12] px-3 text-sm font-black uppercase tracking-[0.16em] text-cfb-cyan"
                    >
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setAccessCode(
                          normalizeBetaAccessCodeSuffix(event.target.value),
                        )
                      }
                  ...
                  <p className="ml-3 text-[11px] font-semibold text-cfb-text-muted">
                    Optional. Enter the six characters after{" "}
                    {BETA_ACCESS_CODE_PREFIX}; this does not control beta
                    access.
                  </p>
                </div>
                {error ? (
                ...
                <Button
                  type="submit"
                  className="h-14 w-full rounded-2xl bg-gradient-to-r from-cfb-cyan to-cfb-brand text-[11px] font-black uppercase tracking-[0.2em] text-slate-950 shadow-[0_18px_42px_hsl(var(--brand-primary)/0.26)] hover:brightness-110"
                  disabled={isLoading}
                >
                  {isLoading ? "Verifying code..." : "Claim free Pro year"}
                </Button>
              </form>
            ) : (
              <form onSubmit={handleSignupSubmit} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <span className="group relative block">
                    <User className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-cfb-text-muted" />
                    <Input
                      id="signup-name"
                      type="text"
                ...
                <div className="space-y-2">
                  ...
                  <span className="group relative block">
                    ...
                    <Input
                      ...
                      value={email}
                      onChange={(event) => setEmail(event.target.value)}
                      readOnly={Boolean(signupHasVerifiedProCode)}
                      required
                    />
                  </span>
                </div>
                <div className="space-y-2">
                  ...
                  <span className="group relative block">
                    <Lock className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-cfb-text-muted" />
                    <Input
                      id="signup-password"
                      type={showPassword ? "text" : "password"}
                      placeholder="••••••••"
                      className="h-14 rounded-2xl border-cfb-border-subtle bg-cfb-surface/80 pl-12 pr-12 text-base font-bold text-cfb-text-primary md:text-sm"
                      value={password}
                    ...
                    <button
                      ...
                      onClick={() => setShowPassword((value) => !value)}
                      className="absolute right-4 top-1/2 inline-flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full text-cfb-text-muted"
                    >
                      {showPassword ? (
                ...
                <p className="rounded-2xl border border-cfb-border-subtle bg-cfb-surface/60 px-4 py-3 text-xs font-semibold text-cfb-text-secondary">
                  {PASSWORD_POLICY_MESSAGE}
                </p>
                {error ? (
                  <div
            ...
            <div className="rounded-2xl border border-cfb-border-subtle bg-cfb-surface/60 p-4">
              <div className="flex items-center gap-3">
                <CalendarClock
                  className="h-5 w-5 text-cfb-gold"
                  aria-hidden="true"
                />
                <p className="text-sm font-semibold text-cfb-text-secondary">
                  {mode === "signin"
                    ? "New here? Create an account with no code required. An Early Access code records a free year of Pro for alpha launch."
                    : mode === "access"
                      ? "A valid code is tied to your email and can be redeemed only once. It does not restrict beta access."
                      : "Already have an account? Sign in. You can optionally claim an Early Access Pro code from this page."}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="relative border-t border-cfb-border-subtle bg-cfb-surface/70 px-6 py-5 text-center">
            {mode === "signin" ? (
              <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-[10px] font-bold uppercase tracking-widest text-cfb-text-secondary">
                <button type="button" onClick={() => selectMode("signup")}>
                  Don&apos;t have an account?
                  <span className="ml-1 font-black text-cfb-gold hover:text-yellow-100">
                    Create one
                  </span>
                </button>
                {betaAccessEnabled ? (
                  <button type="button" onClick={() => selectMode("access")}>
                    Have a code?
                    <span className="ml-1 font-black text-cfb-cyan hover:text-cyan-100">
                      Claim Pro
                    </span>
                  </button>
                ) : null}
        ...
        <DialogContent className="max-w-lg border-cfb-cyan/30 bg-[#081321] text-cfb-text-primary">
          <DialogHeader>
            ...
            <DialogDescription className="text-base font-semibold leading-6 text-cfb-text-secondary">
              Save this password somewhere secure before continuing. CFB Fantasy
              does not store or show it again.
            </DialogDescription>
          </DialogHeader>
          <div className="rounded-2xl border border-cfb-cyan/25 bg-cfb-cyan/[0.08] p-4">
            ...
            <code className="mt-3 block break-all rounded-xl border border-white/15 bg-slate-950/60 px-4 py-3 text-base font-black tracking-[0.08em] text-cyan-100">
              {createdPassword}
            </code>
          </div>
