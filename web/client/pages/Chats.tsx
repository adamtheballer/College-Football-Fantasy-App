...
const errorMessage = (error: unknown) =>
  error instanceof ApiError
    ? error.message
    : "Chat could not be loaded. Please try again.";
...
function threadLabel(thread: ChatThread, currentUserId?: number) {
  ...
  return otherParticipant
    ? `Direct message · ${otherParticipant.display_name}`
    : "Direct message";
}
  ...
  Array.isArray(value)
    ? value.flatMap((asset) => {
        ...
        return [
          {
            player_id:
              typeof asset.player_id === "number" ? asset.player_id : null,
            name: asset.name,
            position:
              typeof asset.position === "string" ? asset.position : null,
...
export function TradeFinalizedCard({
  ...
  const proposingTeam =
    isRecord(metadata.proposing_team) &&
    typeof metadata.proposing_team.name === "string"
      ? metadata.proposing_team.name
      : "Proposing team";
  const receivingTeam =
    isRecord(metadata.receiving_team) &&
    typeof metadata.receiving_team.name === "string"
  ...
  const processingStatus =
    metadata.processing_status === "processed"
      ...
      : "pending_transfer";
  const processAt =
    typeof metadata.players_process_at === "string"
      ? metadata.players_process_at
      : null;
  ...
  const playerList = (assets: TradeAsset[]) =>
    assets.length ? (
      assets.map((asset) => (
        <li key={`${asset.player_id ?? asset.name}-${asset.position ?? ""}`}>
          {asset.name}
          {asset.position ? ` · ${asset.position}` : ""}
          {asset.school ? ` · ${asset.school}` : ""}
        </li>
      ))
    ) : (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-[10px] font-black uppercase tracking-[0.14em] text-muted-foreground">
        <span>
          {processingStatus === "processed"
            ? "Roster transfer complete"
            : processAt
              ? `Roster transfer pending · Players process ${formatTime(processAt)}`
              : "Roster transfer pending"}
        </span>
        {tradePath ? (
          <Link
            to={tradePath}
            className="rounded-lg border border-primary/50 bg-primary/10 px-3 py-2 text-primary transition hover:bg-primary/20"
          >
            View trade
          </Link>
        ) : null}
...
const ownerUserId = (team: unknown): number | null =>
  isRecord(team) && typeof team.owner_user_id === "number"
    ? team.owner_user_id
    : null;
...
export const isPrivateTradeMessage = (
  message: Pick<ChatMessage, "message_type" | "metadata">,
) =>
...
export function PrivateTradeOfferCard({
  ...
  const leagueName =
    isRecord(metadata.league) && typeof metadata.league.name === "string"
  ...
  const isRecipient =
    currentUserId !== undefined && currentUserId === recipientUserId;
  const isProposer =
    currentUserId !== undefined && currentUserId === proposerUserId;
  const tradePath = tradeOfferPath(
    message.league_id,
    metadata.trade_id,
    returnTo ?? `/chats?leagueId=${message.league_id}`,
  ...
  return (
    <div className="space-y-3" data-testid="private-trade-card">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="rounded-full border border-cfb-gold/30 bg-cfb-gold/10 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.14em] text-cfb-gold">
          {statusLabel}
        </span>
      </div>
      <p className="text-xs font-semibold text-foreground">
        {leagueName} · {proposingTeam} ↔ {receivingTeam}
      </p>
      <div className="grid gap-2 sm:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-black/15 p-3">
          <p className="text-[9px] font-black uppercase tracking-[0.14em] text-muted-foreground">
...
export default function Chats() {
  ...
  const [activeThreadByLeague, setActiveThreadByLeague] = useState<
  ...
  useEffect(() => {
    ...
    setSelectedLeagueId((current) => {
      ...
      if (
        activeLeagueId &&
        leagues.some((league) => league.id === activeLeagueId)
      )
  ...
  const chatThreads = useLeagueChatThreads(
  ...
  useEffect(() => {
    const masterThread = threads.find(
      (thread) => thread.thread_type === "league",
    );
    setSelectedThreadId((current) => {
      ...
      if (current && threads.some((thread) => thread.id === current))
        return current;
      const savedThreadId = selectedLeagueId
        ? activeThreadByLeague[selectedLeagueId]
        : null;
      if (
        savedThreadId &&
        threads.some((thread) => thread.id === savedThreadId)
      )
  ...
  useEffect(() => {
    ...
    setActiveThreadByLeague((current) =>
  ...
  const olderMessagePage = useChatMessages(
    selectedLeagueId ?? undefined,
    selectedThreadId ?? undefined,
    olderCursor ?? undefined,
    undefined,
    Boolean(selectedThreadId && olderCursor),
  );
  const sendMessage = useSendChatMessage(
    selectedLeagueId ?? undefined,
    selectedThreadId ?? undefined,
  );
  const markThreadRead = useMarkChatThreadRead(
    selectedLeagueId ?? undefined,
    selectedThreadId ?? undefined,
  );
  const createDirectThread = useCreateDirectChatThread(
    selectedLeagueId ?? undefined,
  );
  ...
  useEffect(() => {
    ...
    setOlderMessages((current) => {
      ...
      return [
        ...olderMessagePage.data.data.filter(
  ...
  const allMessages = useMemo(() => {
    ...
    [...olderMessages, ...messageRows].forEach((message) =>
  ...
  useEffect(() => {
    ...
    if (
      !latestMessage ||
      latestMessage.id === lastMarkedMessageId.current ||
  ...
  const managerOptions = (masterThread?.participants ?? []).filter(
    (member) => member.user_id !== user?.id,
  );
  const selectedThreadType =
    selectedThread?.thread_type === "league" ? "League chat" : "Direct message";
  ...
  const handleRetry = (message: ChatMessage) => {
    if (!message.body || !message.client_message_id || sendMessage.isPending)
      return;
    sendMessage.mutate({
      body: message.body,
  ...
  const oldestMessageId =
    allMessages.find((message) => message.id > 0)?.id ?? null;
  const canLoadOlder = Boolean(
    messages.data?.next_before_message_id ||
    olderMessagePage.data?.next_before_message_id,
  );
  const handleLoadOlder = () => {
    if (oldestMessageId && !olderMessagePage.isFetching)
      setOlderCursor(oldestMessageId);
  };
    ...
    <div className="mx-auto max-w-7xl space-y-5 pb-24 pt-6 lg:pb-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="space-y-2">
          <h1 className="bg-gradient-to-br from-white via-white to-primary/40 bg-clip-text text-5xl font-black uppercase italic tracking-tighter text-transparent">
            League Chat
          </h1>
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-muted-foreground/70">
            League conversation, manager messages, and binding events
          </p>
        </div>
        <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-[9px] font-black uppercase tracking-[0.14em] text-muted-foreground">
          <RefreshCw
            className={cn(
              ...
              (chatThreads.isFetching || messageUpdates.isFetching) &&
                "animate-spin",
            )}
          />{" "}
          Live updates
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger className="h-12 rounded-xl border-white/10 bg-white/[0.03] text-[11px] font-black uppercase tracking-[0.12em]">
                <SelectValue
                  placeholder={
                    leaguesLoading ? "Loading leagues..." : "Select league"
                  }
                />
              </SelectTrigger>
              <SelectContent>
      ...
      {!selectedLeagueId ? (
        ...
        <Card className="rounded-[2rem] border border-red-400/40 bg-red-950/20">
          <CardContent className="p-8 text-sm text-red-100">
            {errorMessage(chatThreads.error)}
          </CardContent>
        </Card>
      ) : (
        <div className="grid min-h-[680px] gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
          <Card className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-card/45">
            <CardHeader className="flex-row items-center justify-between border-b border-white/10 px-5 py-4">
              <CardTitle className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.22em] text-primary">
                <Users className="h-4 w-4" />
                Threads
              </CardTitle>
              <button
                type="button"
                onClick={() => setIsNewMessageOpen((open) => !open)}
                className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-primary/50 bg-primary/10 px-2.5 text-[9px] font-black uppercase tracking-[0.12em] text-primary hover:bg-primary/20"
              >
            ...
            <CardContent className="space-y-4 p-3">
              {isNewMessageOpen ? (
                <div className="rounded-xl border border-primary/30 bg-primary/[0.06] p-3">
                  <p className="mb-2 text-[9px] font-black uppercase tracking-[0.16em] text-primary">
                    Message a league manager
                  </p>
                  <div className="max-h-56 space-y-1 overflow-y-auto">
                    {managerOptions.map((member) => (
                      <button
                        key={member.user_id}
                        type="button"
                        onClick={() => handleCreateDirectThread(member.user_id)}
                        disabled={createDirectThread.isPending}
                        className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left hover:bg-white/[0.06] disabled:opacity-50"
                      >
                        <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-primary/20 text-[9px] font-black text-primary">
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="block truncate text-[10px] text-muted-foreground">
                            {member.fantasy_team_name ?? "League manager"}
                          </span>
                        </span>
                      </button>
                    ))}
                    {!managerOptions.length ? (
                      <p className="px-2 py-3 text-xs text-muted-foreground">
                        No other active managers yet.
                      </p>
                    ) : null}
                  </div>
                  {createDirectThread.isError ? (
                    <p className="mt-2 text-xs text-red-200">
                      {errorMessage(createDirectThread.error)}
                    </p>
                  ) : null}
                </div>
              ) : null}
              ...
              <div className="space-y-1">
                {threads.map((thread) => {
                  ...
                  const title =
                    thread.thread_type === "league"
                      ? `# ${thread.title || "General"}`
                      : (participant?.display_name ?? "Direct message");
                  const preview =
                    thread.last_message_preview ??
                    (thread.thread_type === "league"
                      ? "All current league members"
                      : (participant?.fantasy_team_name ??
                        "Private league conversation"));
                  return (
                    <button
                      key={thread.id}
                      type="button"
                    ...
                    >
                      <span
                        className={cn(
                          "inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[10px] font-black",
                          thread.thread_type === "league"
                      ...
                      >
                        {thread.thread_type === "league"
                          ? "#"
                          : initials(participant?.display_name ?? "DM")}
                      </span>
                      ...
                      <span className="flex shrink-0 flex-col items-end gap-1">
                        ...
                        {thread.unread_count > 0 ? (
                          <span className="inline-flex min-h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1.5 text-[9px] font-black text-white">
          ...
          <Card className="flex min-h-[680px] flex-col overflow-hidden rounded-[1.75rem] border border-white/10 bg-card/45">
            <CardHeader className="border-b border-white/10 px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <CardTitle className="truncate text-sm font-black uppercase tracking-[0.16em] text-foreground">
                    {selectedThread
                      ? threadLabel(selectedThread, user?.id)
                      : "Select a thread"}
                  </CardTitle>
                  {selectedThread && selectedLeague ? (
                    <p className="mt-1 truncate text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">
                      {selectedLeague.name} · {selectedThreadType}
                      {directParticipant
                        ? ` · ${directParticipant.fantasy_team_name ?? directParticipant.display_name}`
                        : ""}
                    </p>
                  ) : null}
                </div>
            ...
            <CardContent className="flex min-h-0 flex-1 flex-col p-0">
              <div className="flex-1 space-y-3 overflow-y-auto p-5">
                {canLoadOlder ? (
                  <div className="flex justify-center">
                    <button
                      ...
                      disabled={olderMessagePage.isFetching}
                      className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-[9px] font-black uppercase tracking-[0.14em] text-muted-foreground hover:text-foreground disabled:opacity-50"
                    >
                      {olderMessagePage.isFetching ? (
                ...
                {messages.isError ? (
                  <p className="rounded-xl border border-red-400/40 bg-red-950/20 p-4 text-sm text-red-100">
                ...
                allMessages.length === 0 ? (
                  <div className="py-20 text-center">
                    <MessageSquare className="mx-auto h-8 w-8 text-primary/60" />
                  ...
                  <div key={message.id} className="space-y-3">
                    ...
                    <div
                      className={cn(
                        ...
                        isSystemMessage(message.message_type)
                          ? "mx-auto max-w-[96%] border-cfb-gold/30 bg-cfb-gold/10"
                          : message.sender_user_id === user?.id
                            ? "ml-auto border-primary/40 bg-primary/15"
                            : "border-white/10 bg-white/[0.04]",
                      )}
                    >
                      <div className="flex items-center justify-between gap-4 text-[9px] font-black uppercase tracking-[0.13em] text-muted-foreground/70">
                        <span>
                          {isSystemMessage(message.message_type)
                            ...
                            : message.sender_user_id === user?.id
                              ...
                              : (message.sender_display_name ??
                                `Manager #${message.sender_user_id}`)}
                          {message.sender_fantasy_team_name
                            ? ` · ${message.sender_fantasy_team_name}`
                            : ""}
                        </span>
                        <span>{formatTime(message.created_at)}</span>
                      </div>
                      {isPrivateTradeMessage(message) ? (
                        <div className="mt-2">
                          <PrivateTradeOfferCard
                            message={message}
                            currentUserId={user?.id}
                            returnTo={`/chats?leagueId=${selectedLeagueId}&threadId=${selectedThreadId}`}
                          />
                        </div>
                      ...
                      {message.delivery_status === "sending" ? (
                        <p className="mt-2 flex items-center gap-1 text-[9px] font-black uppercase tracking-[0.12em] text-primary">
                          <LoaderCircle className="h-3 w-3 animate-spin" />
                          Sending
                        </p>
                      ) : null}
                      {message.delivery_status === "failed" ? (
                        <div className="mt-2 flex items-center justify-between gap-2 text-[9px] font-black uppercase tracking-[0.12em] text-red-200">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-md border border-red-300/40 px-2 py-1 hover:bg-red-500/10"
                          >
              ...
              <form
                onSubmit={handleSend}
                className="border-t border-white/10 bg-black/10 p-4"
              >
                {sendMessage.isError ? (
                  <p className="mb-3 rounded-xl border border-red-400/40 bg-red-950/20 px-3 py-2 text-xs text-red-100">
                ...
                <div className="flex gap-3">
                  <textarea
                    ...
                    className="min-h-14 flex-1 resize-none rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-base text-foreground outline-none transition focus:border-primary/60 disabled:opacity-50 sm:text-sm"
                  />
                  <button
                    type="submit"
                    disabled={
                    ...
                    className="inline-flex h-14 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-[10px] font-black uppercase tracking-[0.16em] text-primary-foreground transition hover:bg-primary/90 disabled:opacity-50"
                  >
                    <Send className="h-4 w-4" />
                    Send
                  </button>
                </div>
                <div className="mt-2 flex justify-between text-[9px] font-bold uppercase tracking-[0.12em] text-muted-foreground">
                  <span>Enter to send · Shift+Enter for a new line</span>
                  <span
                    className={cn(
                      draftMessage.length >= 1800 && "text-cfb-gold",
                    )}
                  >
