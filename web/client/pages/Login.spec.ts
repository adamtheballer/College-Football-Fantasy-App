...
describe("loginErrorMessage", () => {
  ...
  it("maps account lockout separately from invalid credentials", () => {
    expect(
      loginErrorMessage(new ApiError(423, "account temporarily locked")),
    ).toBe(
  ...
  it("keeps validation and service errors actionable", () => {
    expect(
      loginErrorMessage(new ApiError(422, "email: value is not a valid email")),
    ).toBe("email: value is not a valid email");
