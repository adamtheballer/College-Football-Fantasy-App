...
describe("useSendChatMessage", () => {
  it("reconciles an optimistic message by client_message_id without a duplicate", async () => {
    ...
    const { result } = renderHook(() => useSendChatMessage(3, 14), {
    ...
    await waitFor(() => {
      ...
      expect(page?.data).toEqual([
        expect.objectContaining({ id: 88, client_message_id: "browser-123" }),
      ]);
    });
    expect(state.apiPost).toHaveBeenCalledWith("/leagues/3/chats/14/messages", {
      body: "Good luck",
      client_message_id: "browser-123",
    });
  });
    ...
    await act(async () => {
      await expect(
        result.current.mutateAsync({
          body: "Retry me",
          client_message_id: "browser-124",
        }),
      ).rejects.toThrow("network unavailable");
    });

    const page = queryClient.getQueryData<{
      data: Array<{ body: string; delivery_status?: string }>;
    }>(key);
    expect(page?.data).toEqual([
      expect.objectContaining({ body: "Retry me", delivery_status: "failed" }),
    ]);
  });
});
...
describe("useCreateDirectChatThread", () => {
  it("adds the server-created direct thread to the cache before refetching", async () => {
    ...
    const { result } = renderHook(() => useCreateDirectChatThread(3), {
      wrapper: wrapperFor(queryClient),
    });
    ...
    expect(
      queryClient.getQueryData<{ data: Array<{ id: number }>; total: number }>(
        key,
      ),
    ).toEqual({
      data: [
        { id: 9, thread_type: "league" },
        { id: 14, thread_type: "direct", league_id: 3 },
      ],
      total: 2,
    });
  });
