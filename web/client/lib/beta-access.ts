...
export const betaAccessEnabled =
  import.meta.env.VITE_BETA_ACCESS_ENABLED === "true";
...
export const clearBetaAccessReservation = () =>
  safeSessionRemove(RESERVATION_STORAGE_KEY);
...
export const validateBetaAccess = async (
  email: string,
  code: string,
): Promise<BetaAccessReservation> => {
  const payload = await apiPost<BetaAccessValidationPayload>(
  ...
  if (
    !reservation.token ||
    !reservation.email ||
    Number.isNaN(Date.parse(reservation.expiresAt))
  ) {
    throw new Error(
