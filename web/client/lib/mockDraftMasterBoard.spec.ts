...
describe("mock draft master board fallback", () => {
  ...
  it("keeps Tate Sandell and every kicker outside the first 75 fantasy draft slots", () => {
    const board = buildDraftBoard(mergeMockDraftMasterBoardPlayers([]), mockDraftConfig);
    const tateSandell = board.find((player) => player.name === "Tate Sandell");
    const earliestKicker = Math.min(
      ...board.filter((player) => player.pos === "K").map((player) => player.masterDraftRank)
    );

    expect(tateSandell?.masterDraftRank).toBeGreaterThanOrEqual(75);
    expect(earliestKicker).toBeGreaterThanOrEqual(75);
  });
});
