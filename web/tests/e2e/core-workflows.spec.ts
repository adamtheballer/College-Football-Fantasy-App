...
test.describe("critical browser workflows", () => {
  test("Saturday Pick 6 dashboard action opens the contest instead of league creation", async ({
    page,
  }) => {
    ...
    await page.goto("/");
    await page
      .getByRole("link", { name: "Make Your Pick", exact: true })
      .click();

    await expect(page).toHaveURL(/\/saturday-pick-6$/);
    await expect(
      page.getByRole("heading", { name: "MAKE YOUR PICK", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "BUILD YOUR LEAGUE", exact: true }),
    ).not.toBeVisible();
  });

  test("locking a Saturday Pick 6 selection stays in the contest and confirms the pick", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    ...
    await page
      .getByRole("button", { name: "Choose player", exact: true })
      .click();
    await page
      .getByRole("button", { name: "Lock In Pick", exact: true })
      .click();

    await expect(page).toHaveURL(/\/saturday-pick-6$/);
    await expect(
      page.getByText("Your pick is in", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Your pick is in. Follow Ahmad Hardy this Saturday.", {
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      page
        .getByText(
          /Your pick can be changed until Ahmad Hardy's game starts at/,
        )
        .first(),
    ).toBeVisible();
    await expect(
  ...
  test("Saturday Pick 6 finalization marks a losing pick and shows the winner reward state", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    let winner = false;
    const baseContest = {
      ...
      players: [
        ...
        },
        {
          ...
          player_id: 102,
          canonical_position: "RB",
          player_name: "Rival Runner",
          school: "Texas",
          opponent: "Ohio State",
          game_time: "2026-09-05T17:00:00Z",
          image_url: null,
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          entry: {
            ...
            submitted_at: "2026-09-01T12:00:00Z",
            is_winner: winner,
            reward_unlocked_at: winner ? "2026-09-05T21:00:00Z" : null,
          },
    ...
    await page.goto("/saturday-pick-6");
    await expect(
      page.getByRole("heading", { name: "Not this week", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Ahmad Hardy did not finish first. Try again next week."),
    ).toBeVisible();
    ...
    await page.reload();
    await expect(
      page.getByRole("heading", { name: "You got it right", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Your reward code is being prepared."),
    ).toBeVisible();
  });

  test("mobile dashboard retains the normal page scroller outside draft rooms", async ({
    page,
  }, testInfo) => {
    await seedAuthenticatedSession(page);
    await page.route("**/leagues?**", async (route) => {
    ...
    await page.route("**/saturday-pick-6/current?**", async (route) => {
      await route.fulfill({
        status: 404,
        contentType: "application/json",
        body: JSON.stringify({ detail: "No active contest" }),
      });
    });
    ...
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: /Good to see you, Codex/i }),
    ).toBeVisible();
    if (process.env.CAPTURE_MOBILE_UI === "1") {
      await page.screenshot({
        path: testInfo.outputPath("mobile-home-390x844.png"),
        fullPage: false,
      });
    }

    const scrollResult = await page
      .locator("main[data-app-scroll='true']")
      .evaluate((element) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Leagues", exact: true }),
    ).toBeVisible();
    await page.waitForTimeout(350);
    if (process.env.CAPTURE_MOBILE_UI === "1") {
      await page.screenshot({
        path: testInfo.outputPath("mobile-leagues-390x844.png"),
        fullPage: false,
      });
    }

    const leagueRouteResult = await page
      .locator("main[data-app-scroll='true']")
      .evaluate((element) => {
        const area = element as HTMLElement;
    ...
    expect(leagueRouteResult).toEqual({
      owner: "page",
      scrollTop: 0,
    ...
    await expect(page.locator("main[data-app-scroll='true']")).toHaveAttribute(
      "data-scroll-owner",
      "page",
    );
    ...
    const postDraftScrollResult = await page
      .locator("main[data-app-scroll='true']")
      .evaluate((element) => {
        const area = element as HTMLElement;
        area.scrollTop = area.scrollHeight;
    ...
    expect(postDraftScrollResult).toEqual({
      owner: "page",
  ...
  test("login flow stores auth session and routes to dashboard", async ({
    page,
  }) => {
    ...
    const token = await page.evaluate(() =>
      window.localStorage.getItem("cfb_access_token"),
    );
    const user = await page.evaluate(() =>
      window.localStorage.getItem("cfb_user"),
    );
    expect(token).toBe("mock-access-token");
    expect(user).toContain("coach@example.com");
  });

  test("signup is open without a beta code and preserves the authenticated session", async ({
    page,
  }) => {
    const readyUser = {
    ...
    await expect(
      page.getByText("Save this password somewhere secure before continuing."),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", { name: /^Leagues$/i }),
    ).toBeVisible();
  ...
  test("leagues page renders backend response for authenticated session", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    ...
    await expect(
      page.getByRole("heading", { name: /^Draft Countdown$/i }),
    ).toBeVisible();
  });

  test("invalid bootstrap session forces logout and redirects protected routes to login", async ({
    page,
  }) => {
    await page.addInitScript((payload) => {
      window.localStorage.setItem(
  ...
  test("create league workflow posts to backend and opens league hub", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    ...
    await expect(
      page.getByRole("heading", { name: /Build your league/i }),
    ).toBeVisible();
    await page
      .getByRole("button", { name: "Continue to Settings", exact: true })
      .click();
    await expect(
      page.getByRole("heading", { name: "League Settings", exact: true }),
    ).toBeVisible();
    await page
      .getByRole("button", { name: "Continue to Draft", exact: true })
      .click();
    await expect(
      page.getByRole("heading", { name: "Draft Schedule", exact: true }),
    ).toBeVisible();
    await page
    ...
    const createLeagueButton = page.getByRole("button", {
    ...
    await expect(
      page.getByRole("heading", { name: /Invite managers/i }),
    ).toBeVisible();
  ...
  test("join-by-code flow previews league and joins with backend response", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    ...
    const leagueDetail = {
      ...
      members: [
        ...
        },
        {
          ...
          user_id: 42,
          role: "manager",
          joined_at: "2026-03-02T10:01:00Z",
        },
      ],
    };
    ...
    await expect(
      page.getByText(/Draft room access stays locked/i),
    ).toBeVisible();
  });

  test("draft-room pick mutation updates persisted draft state in UI", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    ...
    await expect(
      page.getByRole("heading", { name: /Draft Test League/i }),
    ).toBeVisible();
    for (const viewport of [
      ...
      const draftButton = row.getByRole("button", {
        name: /^Draft Arch Manning$/i,
      });
      await expect(draftButton).toBeVisible();
      await expect(
        row.getByRole("button", { name: /^Queue Arch Manning$/i }),
      ).toHaveCount(0);
      const geometry = await row.evaluate((element) => ({
        rowRight: element.getBoundingClientRect().right,
        documentWidth: document.documentElement.scrollWidth,
      }));
      const draftHeight = await draftButton.evaluate(
    ...
    const desktopRow = page
      ...
      .filter({ hasText: "Arch Manning" });
    await expect(
      desktopRow.getByRole("button", { name: /^Draft Arch Manning$/i }),
    ).toBeVisible();
    await expect(
      desktopRow.getByRole("button", { name: /^Queue Arch Manning$/i }),
    ).toHaveCount(0);
    await page.setViewportSize({ width: 430, height: 932 });
    const draftScrollContract = await page.evaluate(() => {
      const appScroller = document.querySelector<HTMLElement>(
        "main[data-app-scroll='true']",
      );
      const playerList = document.querySelector<HTMLElement>(
        "[data-testid='draft-player-list']",
      );
      ...
      const filterButtons = Array.from(
        document.querySelectorAll<HTMLButtonElement>(
          "[data-testid='draft-player-filters'] button",
        ),
      );
      ...
      return {
        ...
        outerScrollable: appScroller
          ? appScroller.scrollHeight > appScroller.clientHeight
          : null,
        listScrollable: playerList
          ? /auto|scroll/.test(getComputedStyle(playerList).overflowY)
          : null,
        tabsInViewport: tabs
          ? tabs.getBoundingClientRect().bottom <= viewportHeight
          : null,
        filtersOnOneLine:
          new Set(
        ...
        tabLabelsDoNotWrap: Array.from(
          document.querySelectorAll("[data-testid='draft-room-tabs'] button"),
        ).every((button) => getComputedStyle(button).whiteSpace === "nowrap"),
      };
    });
    ...
    const queuedRow = page
      .getByTestId("draft-player-row")
      .filter({ hasText: "Quinn Ewers" });
    await expect(
      page.getByTestId("draft-player-row").filter({ hasText: "Arch Manning" }),
    ).toHaveCount(0);
    await expect(queuedRow).toHaveText(/^2/);
    await expect(page.getByText(/^Draft Complete$/i)).toHaveCount(0);
    await expect(
      queuedRow.getByRole("button", { name: /^Draft Quinn Ewers$/i }),
    ).toHaveCount(0);
    await queuedRow
      .getByRole("button", { name: /^Queue Quinn Ewers$/i })
      .click();
    await expect(
      queuedRow.getByRole("button", { name: /Remove Quinn Ewers from queue/i }),
    ).toBeVisible();
  });

  test("a completed draft stops the timer and offers one clear roster exit", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    ...
    await page.route("**/leagues/1/draft-room", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(completedDraftRoom),
      });
    });
    await page.route("**/players?**", async (route) => {
      await route.fulfill({
    ...
    await expect(
      completionDialog.getByText(/Rosters finalized/i),
    ).toBeVisible();
    await expect(page.getByText("Pick Timer", { exact: true })).toHaveCount(0);

    await completionDialog
      .getByRole("button", { name: "Stay in Draft Room" })
      .click();
    ...
    await expect(completionDialog).toBeVisible();
    await completionDialog
      .getByRole("button", { name: "View Your Roster" })
      .click();
    ...
    await expect(page).toHaveURL(/\/league\/1\/roster$/);
    await expect(
      page.getByText("Loading draft lobby...", { exact: true }),
    ).toHaveCount(0);
    await expect(page.getByRole("link", { name: /^Draft$/i })).toHaveCount(0);
  });

  test("league matchup page renders projected teams and honest empty state", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
                ...
                data: [
                  {
                    ...
                    week: 1,
                    status: "projected",
                    home_team_name: "Emily's Team",
                    away_team_name: "Adam 2's Team",
                    home_score: 133.1,
                    away_score: 137.0,
                  },
                  {
    ...
    await expect(
      page.getByRole("heading", { name: /^Matchup$/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Emily's Team vs Adam 2's Team" }),
    ).toBeVisible();
    ...
    await expect(
      page
        .getByText("Projected Leader")
        .locator("..")
        .getByText("Adam 2's Team"),
    ).toBeVisible();
    await expect(page.getByTestId("win-chance-left-bar")).toHaveAttribute(
      "style",
      /width: 48\.05%/,
    );
    await expect(page.getByTestId("win-chance-right-bar")).toHaveAttribute(
      "style",
      /width: 51\.95%/,
    );
    ...
    await expect(
      page.getByRole("button", { name: /Arch Manning/ }),
    ).toBeVisible();
    ...
    await expect(page.getByTestId("compact-win-chance-left-bar")).toHaveClass(
      /from-rose-800/,
    );
    await expect(page.getByTestId("compact-win-chance-right-bar")).toHaveClass(
      /from-emerald-700/,
    );
    ...
    await page.screenshot({
      path: "test-results/mobile-matchup-linear-meter.png",
      fullPage: true,
    });
    ...
    await expect(page.getByText("48.1% / 51.9%")).toBeVisible();
    await page
      .getByRole("combobox", { name: "League matchup" })
      .selectOption("102");
  ...
  test("league settings show every pre-scoring team at 0-0", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    ...
    await expect(
      page.getByText("Standings are not available yet.", { exact: true }),
    ).toHaveCount(0);

    await page
      .getByRole("button", { name: "Point System", exact: true })
      .click();
    await expect(
      page.getByText("Roster System", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Waiver System", { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Weeks 1–13 · every team has one matchup each week."),
    ).toBeVisible();
    const weekSelector = page.getByRole("group", {
      name: "Regular season week",
    });
    const weekThirteen = weekSelector.getByRole("button", {
      name: "Week 13",
      exact: true,
    });
    ...
    await page
      ...
      .click();
    await expect(
      page.getByText("Managers: Adam and Guy", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Codex Team sent", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Arch Manning · QB · Texas", { exact: true }),
    ).toBeVisible();
    const viewTrade = page.getByRole("link", {
      name: "View trade",
      exact: true,
    });
  ...
  test("league settings keeps draft results available before the draft begins", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    await page.addInitScript(() => {
    ...
    await page
      .getByRole("button", { name: "Draft Results", exact: true })
      .click();
    await expect(
      page.getByRole("heading", { name: "Draft Results", exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText(
        "No completed draft picks yet. Results will appear here as soon as the draft begins.",
      ),
    ).toBeVisible();
  });

  test("manage roster can view every league team while only the owner can manage their lineup", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    await page.addInitScript(() => {
    ...
    await expect(
      page.getByText("Managing your roster", { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Viewing league roster", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Rival Team · 0-0-0 · Read-only", { exact: true }),
    ).toBeVisible();
  });

  test("trade builder requires fresh analysis before sending an offer", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    await page.addInitScript(() => {
    ...
    await page.route("**/teams/11/roster**", async (route) => {
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          data: [rosterEntry(1, 11, 201, "Arch Manning")],
          total: 1,
          limit: 50,
          offset: 0,
        }),
    ...
    await page.route("**/teams/12/roster**", async (route) => {
      await route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          data: [rosterEntry(2, 12, 301, "Rival QB")],
          total: 1,
          limit: 50,
          offset: 0,
        }),
    ...
    await page.setViewportSize({ width: 390, height: 844 });
    await expect(
      reviewDialog.getByRole("button", { name: "Close" }),
    ).toBeVisible();
    ...
    await expect(
      reviewDialog.getByRole("button", { name: /^Send Final Trade$/i }),
    ).toBeEnabled();
    await reviewDialog
      .getByRole("button", { name: /^Send Final Trade$/i })
      .click();
        ...
        body: JSON.stringify({
          ...
          items: [
            ...
            },
            {
              ...
              draft_pick_id: null,
              item_type: "player",
              player_name: "Rival QB",
              player_position: "QB",
              player_school: "Oregon",
            },
          ],
        }),
      });
    ...
    await page.goto(
      "/leagues/1/trades/77?returnTo=%2Fchats%3FleagueId%3D1%26threadId%3D9",
    );
    const focusedOffer = page.getByRole("dialog", {
      name: /Review Trade Offer/i,
    });
    await expect(focusedOffer).toBeVisible();
    await expect(
      focusedOffer.getByText("Arch Manning", { exact: true }),
    ).toBeVisible();
    await focusedOffer
      .getByRole("button", { name: "Back to league chat", exact: true })
      .click();
    await expect(page).toHaveURL(/\/chats\?leagueId=1&threadId=9$/);
  });

  test("single-player mock draft stays local and resets without real roster mutation", async ({
    page,
  }, testInfo) => {
    await seedAuthenticatedSession(page);
    ...
    const players = Array.from({ length: 140 }, (_, index) => {
      ...
      return {
        ...
        name:
          rank === 122
            ? "Jeremiah Smith"
            : `Mock ${position} ${String(rank).padStart(3, "0")}`,
        position,
    ...
    await page.route("**/leagues/**/draft-picks", async (route) => {
      ...
      await route.fulfill({
        status: 500,
        contentType: "application/json",
        body: JSON.stringify({ detail: "real draft mutation blocked" }),
      });
    });
    await page.route("**/teams/**/roster", async (route) => {
      ...
      await route.fulfill({
        status: 500,
        contentType: "application/json",
        body: JSON.stringify({ detail: "real roster mutation blocked" }),
      });
    });
    await page.route("**/mock-drafts**", async (route) => {
      ...
      await route.fulfill({
        status: 500,
        contentType: "application/json",
        body: JSON.stringify({ detail: "backend mock draft blocked" }),
      });
    });
    await page.route("**/stats/teams?**", async (route) => {
      await route.fulfill({
    ...
    for (const viewport of [
      ...
      const row = page
        .getByTestId("draft-player-row")
        .filter({ hasText: "Jeremiah Smith" });
      await expect(row).toBeVisible();
      const queueButton = row.getByRole("button", {
        name: /^Queue Jeremiah Smith$/i,
      });
      await expect(queueButton).toBeVisible();
      await expect(
        row.getByRole("button", { name: /^Draft Jeremiah Smith$/i }),
      ).toHaveCount(0);
      const geometry = await row.evaluate((element) => ({
        rowRight: element.getBoundingClientRect().right,
        documentWidth: document.documentElement.scrollWidth,
      }));
      const queueHeight = await queueButton.evaluate(
        (element) => element.offsetHeight,
      );
      expect(geometry.documentWidth).toBeLessThanOrEqual(viewport.width);
      expect(geometry.rowRight).toBeLessThanOrEqual(viewport.width);
      ...
      if (process.env.CAPTURE_MOBILE_UI === "1") {
        await page.screenshot({
          path: testInfo.outputPath(
            `mobile-mock-draft-${viewport.width}x${viewport.height}.png`,
          ),
          fullPage: false,
        });
      }
    ...
    const desktopRow = page
      ...
      .filter({ hasText: "Jeremiah Smith" });
    await expect(
      desktopRow.getByRole("button", { name: /^Queue Jeremiah Smith$/i }),
    ).toBeVisible();
    await expect(
      desktopRow.getByRole("button", { name: /^Draft Jeremiah Smith$/i }),
    ).toHaveCount(0);
    await page.setViewportSize({ width: 430, height: 932 });
    const scrollContract = await page.evaluate(() => {
      ...
      return {
        ...
        listScrollable: playerList
          ...
          : null,
        tabsInViewport: tabs
          ? tabs.getBoundingClientRect().bottom <= window.innerHeight
          : null,
    ...
    await page.evaluate(() =>
      document
        .querySelector<HTMLElement>("main[data-app-scroll='true']")
    ...
    expect(
      playerRequests.every((request) => request.draftEligible === "true"),
    ).toBe(true);
    ...
    const beforeUserPick = await page.evaluate(() =>
      JSON.parse(
        window.localStorage.getItem("cfb_single_player_mock_draft") ?? "{}",
      ),
    );
    expect(beforeUserPick.picks).toHaveLength(beforeUserPick.userTeamId - 1);
    expect(
      beforeUserPick.picks.every(
        (pick: { pickedBy: string }) => pick.pickedBy === "bot",
      ),
    ).toBe(true);
        ...
        page.evaluate(
          () =>
            JSON.parse(
              ...
              (pick: { pickedBy: string }) => pick.pickedBy === "user",
            ) ?? false,
        ),
      )
      .toBe(true);

    const afterUserPick = await page.evaluate(() =>
      JSON.parse(
        window.localStorage.getItem("cfb_single_player_mock_draft") ?? "{}",
      ),
    );
    const userPickIndex = afterUserPick.picks.findIndex(
      (pick: { pickedBy: string }) => pick.pickedBy === "user",
    );
    ...
    const afterCpuPick = await page.evaluate(() =>
      JSON.parse(
        window.localStorage.getItem("cfb_single_player_mock_draft") ?? "{}",
      ),
    );
    ...
    await page
      .getByRole("button", { name: `Open ${rosterPlayerName} player card` })
      .click();
    await expect(
      page.getByRole("dialog", { name: `${rosterPlayerName} player card` }),
    ).toBeVisible();
  ...
  test("mock draft reconciles a legacy alias so a drafted player cannot remain on the board", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    await page.addInitScript(() => {
    ...
    await expect
      .poll(() =>
        page.evaluate(() => {
          ...
          return stored.picks?.map(
            (pick: { playerId: number; school: string }) => [
              pick.playerId,
              pick.school,
            ],
          );
  ...
  test("watchlist create/add/remove persists through backend contracts", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
    ...
    await page.route("**/watchlists**", async (route) => {
      ...
      if (method === "POST" && path.match(/\/watchlists\/\d+\/players$/)) {
        ...
        if (!target) {
          await route.fulfill({
            status: 404,
            contentType: "application/json",
            body: JSON.stringify({ detail: "watchlist not found" }),
          });
          return;
        }
      ...
      if (
        method === "DELETE" &&
        path.match(/\/watchlists\/\d+\/players\/\d+$/)
      ) {
        const target = watchlists[0];
        if (!target) {
          await route.fulfill({
        ...
        target.players = target.players.filter(
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: /^Available Players$/i }),
    ).toBeVisible();

    const archManningRow = page
      .getByText("Arch Manning")
      .locator("xpath=ancestor::tr");
    await archManningRow.getByRole("button", { name: /^Watch$/i }).click();
    await expect(
      archManningRow.getByRole("button", { name: /^Watching$/i }),
    ).toBeVisible();
    ...
    await expect(
      mobileRow.getByRole("button", {
        name: /Remove Arch Manning from watchlist/i,
      }),
    ).toBeVisible();
    expect(
      await page.evaluate(
    ...
    await page.screenshot({
      path: "test-results/mobile-waivers-compact.png",
      fullPage: true,
    });
    ...
    await page
      .getByRole("button", { name: /Remove Arch Manning from watchlist/i })
      .evaluate((button) => {
  ...
  test("available players page submits a waiver claim without using the legacy add-drop endpoint", async ({
    page,
  }) => {
    await seedAuthenticatedSession(page);
          ...
          available_players: [
            {
              ...
              position: "QB",
              school: "Texas",
              weekly_projected_fantasy_points: 24.0,
            },
            {
              id: 902,
              name: "Ryan Wingo",
    ...
    await page.route("**/teams/**/add-drop", async (route) => {
      ...
      await route.fulfill({
        status: 500,
        contentType: "application/json",
        body: JSON.stringify({ detail: "add/drop disabled" }),
      });
    });
    ...
    await expect(
      page.getByText(/No active or recent waiver claims/i),
    ).toBeVisible();
    const archManningRow = page.getByRole("button", {
      name: "1 Arch Manning Available",
    });
    ...
    const claimDialog = page.getByRole("dialog", {
      name: /Submit Waiver Claim/i,
    });
    await expect(claimDialog).toBeVisible();
    await claimDialog
      .getByRole("button", { name: /Confirm Waiver Claim/i })
      .click();
