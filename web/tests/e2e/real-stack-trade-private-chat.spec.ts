...
async function getDraftablePlayers(
  request: APIRequestContext,
): Promise<{ quarterback: Player; runningBack: Player }> {
  const response = await request.get(
...
test.describe("real FastAPI/PostgreSQL league chat", () => {
  ...
  test("two league members exchange messages and see a binding trade card without cross-league leakage", async ({
    ...
    expect(await rosterPlayerIds(request, userA, averyTeam!.id)).toContain(
      runningBack.id,
    );
    expect(await rosterPlayerIds(request, userB, blakeTeam!.id)).toContain(
      quarterback.id,
    );
