...
test.describe("real two-manager draft lifecycle", () => {
  test.skip(
    !realStackEnabled,
    "Run through npm run test:e2e:real against the isolated Compose stack.",
  );
  test.setTimeout(220_000);

  test("enforces the standard beta roster and keeps two signed-in managers synchronized through timeout auto-picks", async ({
    browser,
  }) => {
    ...
    try {
      ...
      const createResponse = await realApi<{
        league: { id: number };
        invite_code: string;
      ...
      await expect(
        commissioner.getByRole("button", { name: /^Start Draft$/i }),
      ).toBeVisible();
      ...
      await commissioner
        .getByRole("button", { name: /^Start Draft$/i })
        .click();
      await expect
        .poll(async () => {
      ...
      await expect(commissioner.getByText("Pick Timer")).toBeVisible({
        timeout: 15_000,
      });
      await expect(manager.getByText("Pick Timer")).toBeVisible({
        timeout: 15_000,
      });
      ...
      expect(managerRoom.body.picks.map((pick) => pick.player_id)).toEqual(
        room.body.picks.map((pick) => pick.player_id),
      );
      expect(new Set(room.body.picks.map((pick) => pick.player_id)).size).toBe(
        room.body.picks.length,
      );
