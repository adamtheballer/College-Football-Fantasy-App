...
| P1 | Trade browser workflow | The test still asserted a retired linear “Analyze” flow after the approved roster → opponent → review builder replaced it. A later cloud rerun also found an ambiguous `Your roster` locator because the page legitimately renders an `h1` and a section heading with that text. | Rewrote the E2E test to select each roster, review the trade, and assert the submitted payload; constrain the assertion to the page-level `h1`. | Targeted trade E2E passed; full browser E2E passed 25/25; fresh RC cloud CI rerun pending. | FIXED — pending final RC CI |
