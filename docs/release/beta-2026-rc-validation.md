...
| Player-headshot licensing | `PLAYER_HEADSHOTS_ENABLED=false` at profile sync, seed import, API serialization, player card, and Saturday Pick boundaries | Implemented for beta: image columns and existing URLs remain reversible in the database, but no new third-party headshots are imported and all beta API responses provide `null`. The UI retains its same-size neutral silhouette fallback. Full regression CI must pass for this commit. |
