...
SOURCE_PATH = PROJECT_ROOT / "api" / "app" / "data" / "cfb27_ratings_2026-08-05.csv"
...
def load_rating_rows(source_path: Path = SOURCE_PATH) -> list[tuple[int, str, str, str, int]]:
    if source_path.suffix.casefold() == ".csv":
        with source_path.open(newline="", encoding="utf-8-sig") as handle:
            source = [
                {"name": row["PLAYER"], "school": row["TEAM"], "position": row["EA POSITION"], "overall": row["OVR"]}
    ...
    for index, row in enumerate(source, start=1):
        ...
        seen_keys.add(key)
        rows.append((rank, name, school, position, overall, index))
    return [
        (rank, name, school, position, overall)
        for rank, (_source_rank, name, school, position, overall, _source_index) in enumerate(
            sorted(rows, key=lambda row: (-row[4], row[5])), start=1
        )
    ]
...
def render_frontend_module(rows: list[tuple[int, str, str, str, int]]) -> str:
    ...
    return f"""// Generated by scripts/generate_cfb27_frontend.py from api/app/data/cfb27_ratings_2026-08-05.csv.
