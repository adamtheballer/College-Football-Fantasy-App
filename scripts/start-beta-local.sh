...
for _attempt in $(seq 1 60); do
  ...
  if [[ -n "$runtime" ]] && python3 -c 'import json,sys; d=json.loads(sys.stdin.read()); assert d["git_sha"] == sys.argv[1]; assert d["runtime_mode"] == "release_candidate"' "$CFF_GIT_SHA" <<< "$runtime"; then
