...
jq -e --arg sha "$CFF_GIT_SHA" '.git_sha == $sha and .alembic_revision == "0098_profile_photo_data" and .scoring_mode == "disabled" and .sportsdata_enabled == false and .provider_polling_expected == false and .email_enabled == false' <<<"$runtime_payload" >/dev/null
