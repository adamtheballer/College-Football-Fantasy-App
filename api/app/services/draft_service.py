...
def _draft_teams_are_ready(db: Session, league: League, teams: list[Team]) -> bool:
    ...
    return (
        member_count >= league.max_teams
        and len(teams) == league.max_teams
        and all(team.owner_user_id is not None for team in teams)
    )
    ...
    for draft_id in draft_ids:
        ...
        try:
            ...
            if draft_row.status == "pre_draft" and draft_starts_at is not None and draft_starts_at <= current:
                ...
                auto_pick_expired_draft_pick(db, league=league, current_user=current_user, now=current)
            else:
                summary["skipped"] += 1
                continue
        except HTTPException:
