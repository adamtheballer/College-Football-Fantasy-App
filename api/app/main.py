...
def log_runtime_identity_on_startup() -> None:
    ...
    logger.info(
        "api_runtime_started api_process_instance_uuid=%s database_instance_uuid=%s git_sha=%s git_branch=%s environment=%s scoring_mode=%s sportsdata_enabled=%s provider_polling_expected=%s email_enabled=%s readiness_status=%s",
...
app.include_router(admin_schedule.router, prefix="/admin/schedules", tags=["admin-schedules"])
