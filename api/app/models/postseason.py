...
class LeaguePostseasonSettings(TimestampMixin, Base):
    ...
    reseeding_enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
...
class PostseasonBracket(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint("league_id", "season", "bracket_type", name="uq_postseason_bracket_league_season_type"),
    ...
    league_id: Mapped[int] = mapped_column(ForeignKey("leagues.id", ondelete="CASCADE"), nullable=False)
    season: Mapped[int] = mapped_column(Integer, nullable=False)
    bracket_type: Mapped[str] = mapped_column(String(32), nullable=False)
