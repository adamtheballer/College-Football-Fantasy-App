...
class Draft(TimestampMixin, Base):
    ...
    draft_starts_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    current_pick_number: Mapped[int] = mapped_column(Integer, default=0)
    current_pick_started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    current_pick_deadline: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
